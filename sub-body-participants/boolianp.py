 ...
 for dirpath, dirs, files in os.walk("maindatabase/"+inputdir[0]):
    ...
    for filename in fnmatch.filter(files, '*'):
        ...
        with open(os.path.join(dirpath, filename)) as f:
            ...
            filtered_subj = preunique(email['subject'])
            filtered_body = preunique(email.get_payload())
            participants=part(email['cc'])
            participants1=part(email['to'])
            if(participants!=None):
             #filtered = filtered_subj + filtered_body+participants
             counter.update(participants)
            elif(participants1!=None):
            # filtered = filtered_subj + filtered_body+participants1
             counter.update(participants1)
